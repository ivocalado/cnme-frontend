...
export class PoloEditComponent implements OnInit {
    ...
    consultarCep() {
        let cep: string = this.poloForm.controls['localidade'].value.cep
        console.log(cep)
        ...
        this.unidadeDataService.consultarCep(cep).subscribe((res: any) => {
            ...
            this.poloForm.controls['localidade'].patchValue({
                bairro: res.bairro,
                cep: res.cep,
                estado_id: est_id,
    ...
    get isCepInformed() {
        return this.poloForm.controls['localidade'].value.cep
    }
